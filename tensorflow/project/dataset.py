...
def gather_paths(config):
    ...
    """
    Summary:
        gather image path of train, test, valid
    Arguments:
        config (dict): Configuration directory
    Return:
        save file
            ...
            for j in range(patch_weight):   # image row traverse
                ...
                if end <= img.shape[1]:
                    ...
                    if patch_class_balance or test=='test': # for without patch_class_balance or test take all patch images
                        ...
                        if percen["one_class"]>39.0: # take 39% as the threshold for class percentage
                            patch_idx.append([s_row, e_row, start, end])
    return  patch_idx
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(batch_x)):   # get all image and target for single batch
            if self.patchify:   # for patchify is true get imgs and targets
                imgs.append(read_img(batch_x[i], patch_idx = batch_patch[i])) # get image from the directory
...
def get_train_val_dataloader(config):
    ...
    if not (os.path.exists(config["p_train_dir"])) and config['patchify']:  # creating json files for train and validation
